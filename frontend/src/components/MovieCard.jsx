...
const MovieCard = ({ movie }) => {
  ...
  const [isStateWatchlist, setIsStateWatchlist] = useState(isInWatchlist);
  ...
  const handleWatchlistToggle = (e) => {
    ...
    setIsStateWatchlist(!isStateWatchlist);
  };
    ...
    >
      ...
      <button
        ...
        className={`absolute top-2 right-2 w-8 h-8 rounded-full flex items-center justify-center transition-colors ${
          isStateFav ? 'bg-red-600' : 'bg-gray-800 bg-opacity-70 hover:bg-gray-700'
        }`}
        aria-label={isStateFav ? 'Remove from favorites' : 'Add to favorites'}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          fill="currentColor"
          className={`w-5 h-5 ${isStateFav ? 'text-white' : 'text-gray-300'}`}
        >
      ...
      <button
        ...
        className={`absolute top-2 right-12 w-8 h-8 rounded-full flex items-center justify-center transition-colors ${
          isStateWatchlist ? 'bg-blue-600' : 'bg-gray-800 bg-opacity-70 hover:bg-gray-700'
        }`}
        aria-label={isStateWatchlist ? 'Remove from watchlist' : 'Add to watchlist'}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          fill="currentColor"
          className={`w-5 h-5 ${isStateWatchlist ? 'text-white' : 'text-gray-300'}`}
        >
